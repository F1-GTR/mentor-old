...
void deriv2b1012 (const range& r, char task[BUFSZ], char answ[5][BUFSZ], char src[BUFSZ])
{
    ...
    sprintf(answ[0], "y`=(-1/(1+(%s)^2))*.(%s/sqrt(x))", ab, fraction(buf[2],a,1,2));
    sprintf(answ[1], "y`=(-1/(1+(%s)^2))*.(%s/sqrt(x))", ab, fraction(buf[2],a,1,2));
    sprintf(answ[2], "y`=(-1/(1+(%s)^2))*.(%s/sqrt(x))", ab, fraction(buf[2],a,1,2));
    sprintf(answ[3], "y`=(-1/(1+(%s)^2))*.(%s/sqrt(x))", ab, fraction(buf[2],a,1,2));
}
